...
func (repo Agendamento) BuscarAgendamentosDoUsuario(usuarioID uint64) ([]models.Agendamento, error) {
	linhas, erro := repo.db.Query("select a.id, a.propriedade_id, a.dia_agendamento, a.checkin, a.checkout, a.observacoes from agendamentos a INNER JOIN propriedades p ON p.id = a.propriedade_id inner join usuarios u on u.id = p.cliente_id where u.id = ?", usuarioID)
	if erro != nil {
		return nil, erro
	}
	defer linhas.Close()
